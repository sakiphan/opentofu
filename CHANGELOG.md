...
- `tofu plan -generate-config-out=...` no longer fails with an "OpenTofu attempted to process a resource at ... that has no configuration; this is a bug in OpenTofu" error when an import block targets a resource that already exists in the state. The configuration is now generated from the current state instead. ([#4456](https://github.com/opentofu/opentofu/issues/4456))
